...
def wait_for_current_run_start_time(irrigation, program_id, timeout_sec=60, poll_sec=2):
    ...
    while time.time() < timeout:
        ...
        actual_start = ReportParser.parse_actual_start_time(running_report)
...
def wait_for_completed_report(irrigation, program_id, timeout_sec=60, expected_start_time=None):
    timeout = time.time() + timeout_sec
    ...
    while time.time() < timeout:
        ...
        if (expected_start_time and report_start_time and not start_times_match( expected_start_time, report_start_time,)):
            if not stale_report_logged:
                print("Ignoring stale completed report. "
        ...
        is_running_state = report_state_is_running(completed_report)
    ...
    if (latest_finalized_report is not None and active_program_state != "Running" and finalized_mismatch_count >= 3
    ):
        print( "Controller is not running and finalized completed report "
